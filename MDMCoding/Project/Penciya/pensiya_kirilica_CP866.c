...
int BetweenDates(int *dd1, int *mm1, int *yy1, int *dd2, int *mm2, int *yy2) {
  ...
  while (IsNoCorrectDate(*dd1, *mm1, *yy1, *dd2, *mm2, *yy2)) {
    printf("\nОшибка!!!Конечная дата меньше или равна начальной!!!\n");
...
// ************************************************************************
  ...
  while (nadb_za_vyslugu<1 || nadb_za_vyslugu>1000000) {
    printf("\nВведите надбавку за выслугу лет из зарплатной квитации\n"
          "(Пример \"10000.00\"): ");
...
printf("Оклад по должности         = %.2f р\n", oklad_dolzhn);
printf("Оклад по званию            = %.2f р\n", oklad_zvan);
printf("Надбавка за выслугу лет    = %.2f р\n", nadb_za_vyslugu);
printf("Оклад денежного содержания = %.2f р\n", oklad_denezh_soderzh);
...
if (pens_koeff == 0.0) {
  ...
  printf("\n!!!Пенсия без учета ветерана боевых действий и др.!!!\n");
}
